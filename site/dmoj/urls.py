...
register_patterns = [
    ...
    path('admin-login/', user.AdminLoginView.as_view(), name='admin_login'),
    ...
    path('id/find/complete/', user.IdFindCompleteView.as_view(template_name='registration/id_find_complete.html',), name='id_find_complete'), 
    
    # id/pw 찾기 기능 오류 대비 라우팅
    path('idpw/find/error/',user.IdpwFindErrorView.as_view(template_name='registration/idpw_find_error.html',),name='idpw_find_error'),
...
urlpatterns = [
    ...
    path('problem/<str:problem>', include([
        ...
        # best solution should be deleted
        ...
        path('/preview', TestCasePreView.as_view(), name='testcase_preview' ),
    ...
    # path('practices/past', paged_list_view(practices.ContestPastList, 'practice_past_list')),
    ...
    #     path('/', lambda _, practice: HttpResponsePermanentRedirect(reverse('practice_view', args=[contest]))),
    # ])),
    ## Practice End ##
    ...
    path('contests/<int:is_practice>/', include([
        ...
        path('.ics', contests.ContestICal.as_view(), name='contest_ical'),
        path('<int:year>/<int:month>/', contests.ContestCalendar.as_view(), name='contest_calendar'),
        re_path(r'^tag/(?P<name>[a-z-]+)', include([
    ...
    # # path('contests/spectate', paged_list_view(contests.ContestSpectateList, 'contest_spectate_list')),
    ...
    path('contest/<str:contest>', include([
        ...
        ### new contest/submissions 
        ## 필터 창에서 해당 경로 + .? 와 같은 방식으로 필터를 사용하므로, 끝에 / 추가 필요
        path('/submissions/',
             paged_list_view(submission.AllSubmissions, 'contest_all_user_submissions')),

        ### should delete this three urls
        # path('/rank/<str:problem>/',
        ...
        # JSON으로 대회 결과 출력
        path('/result', contests.ContestDetailJSON.as_view(), name='contest_detail_json'),
        path('/resultdownload', contests.ContestDetailExcelDownload.as_view(), name='contest_detail_excel_download'),
        
        # 코드 다운
        path('/codedownload', contests.ContestDetailCodeDownload.as_view(), name='contest_detail_code_download'),
    ...
    path('api/', include([
        ...
        path('contest/<str:contest_key>/verify-code/', contests.verify_contest_code, name='verify_contest_code'),
    ...
    path('term/', include([
        ...
        path('3', term.term_three_view, name='term_three'),
        path('4', term.term_four_view, name='term_four'),
        path('5', term.term_five_view, name='term_five'),
        path('6', term.term_six_view, name='term_six'),
    ])),
...
favicon_paths = ['apple-touch-icon-180x180.png', 'apple-touch-icon-114x114.png', 'android-chrome-72x72.png',
                 ...
                 'apple-touch-icon-120x120.png', 'mstile-310x310.png', 'main_logo.png', 'title_logo.png']
