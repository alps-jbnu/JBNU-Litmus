...
class SeleniumPDFRender(BasePdfMaker):
    ...
    def _make(self, debug):
        #일부 내용 추가
        service = Service(executable_path=settings.SELENIUM_CHROMEDRIVER_PATH)
        options = webdriver.ChromeOptions()
        options.add_argument('--headless')
        options.add_argument('--no-sandbox')
        options.add_argument('--desable-setuid-sandbox')
        ...
        #     self.log = self.get_log(browser) + '\nPDF math rendering timed out' + self.htmlfile
        #     return
