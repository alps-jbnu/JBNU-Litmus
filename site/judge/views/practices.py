...
class ContestList(QueryStringSortMixin, DiggPaginatorMixin, TitleMixin, ContestListMixin, ListView):
    ...
    title = gettext_lazy(_('과제'))
    title_info = '진행 중 / 진행 예정 과제 목록'
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            for participation in (
                ContestParticipation.objects.filter(virtual__in=[0, -1], user=self.request.profile, contest_id__in=present)
...
class ContestPastList(ContestList):
    template_name = 'practice/pastlist.html'
    title = '과제'
    title_info = '종료된 과제 목록'
    context_object_name = 'past_contests'
...
class ContestMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            if key:
                return generic_message(request, _('No such practice'),
                                       _('키 "%s"에 해당하는 과제를 찾을 수 없습니다.') % key)
            else:
                return generic_message(request, _('No such practice'),
                                       _('해당 과제를 찾을 수 없습니다.'))
        except PrivateContestError as e:
            return render(request, 'practice/private.html', {
                'error': e, 'title': _('"%s" 과제 접근이 거부되었습니다.') % e.name,
            }, status=403)

# contest(practice) 상세 페이지
class ContestDetail(ContestMixin, TitleMixin, CommentedDetailView):
    template_name = 'practice/contest.html'
...
class ContestClone(ContestMixin, PermissionRequiredMixin, TitleMixin, SingleObjectFormView):
    title = gettext_lazy(_('과제 복사'))
    template_name = 'practice/clone.html'
...
class ContestJoin(LoginRequiredMixin, ContestMixin, SingleObjectMixin, View):
    ...
    def join_contest(self, request, access_code=None):
        ...
        return HttpResponseRedirect(reverse('practice_view', args=[contest]))
    ...
    def ask_for_access_code(self, form=None):
        ...
        return render(self.request, 'practice/access_code.html', {
...
class ContestCalendar(TitleMixin, ContestListMixin, TemplateView):
    firstweekday = SUNDAY
    template_name = 'practice/calendar.html'
...
class ContestStats(TitleMixin, ContestMixin, DetailView):
    template_name = 'practice/stats.html'
...
def contest_ranking_ajax(request, contest, participation=None):
    ...
    return render(request, 'practice/ranking-table.html', {
        'users': users,
        'problems': problems,
        'contest': contest,
...
class ContestRankingBase(ContestMixin, TitleMixin, DetailView):
    template_name = 'practice/ranking.html'
    tab = None
...
class ContestParticipationList(LoginRequiredMixin, ContestRankingBase):
    ...
    def get_ranking_list(self):
        ...
        live_link = format_html('<a href="{2}#!{1}">{0}</a>', _('Live'), self.profile.username,
                                reverse('practice_ranking', args=[self.object.key]))
...
class ContestTagDetailAjax(DetailView):
    ...
    template_name = 'practice/tag-ajax.html'
