...
class SubmissionsListBase(DiggPaginatorMixin, TitleMixin, ListView):
    ...
    def _get_queryset(self):
        ...
        if self.in_contest:
            # 자신이 참가했던 모든 대회 리스트
            all_contests = ContestParticipation.objects.filter(
                user=self.request.profile,
                virtual=ContestParticipation.LIVE  # 실제 참가한 대회만
            ).select_related('contest').values_list('contest', flat=True)

            # 본인의 제출 기록 필터
            user_submissions_filter = Q(user=self.request.profile)

            # 참가했던 모든 대회 순회
            for contest_id in all_contests:
                contest = Contest.objects.get(id=contest_id)
                # 점수판을 볼 수 있는 경우: 해당 대회의 모든 제출 기록 추가
                if contest.can_see_full_scoreboard(self.request.user):
                    queryset = queryset | Submission.objects.filter(contest_object=contest)
                # 점수판을 볼 수 없는 경우: 해당 대회의 본인 제출 기록만 추가
                else:
                    queryset = queryset | Submission.objects.filter(contest_object=contest, user=self.request.profile)
        else:
        ...
        if self.selected_problems:
            queryset = queryset.filter(problem__in=Problem.objects.filter(code__in=self.selected_problems))
    ...
    def get_searchable_problems(self):
        ...
        if not self.request.user.is_superuser and not self.request.user.is_staff:
            if self.in_contest:
                return self.contest.contest_problems.all().values_list('problem__code', 'problem__name')
            else:
                return Problem.objects.none().values_list('code', 'name')
        # 교수는 모든 문제를 검색할 수 있다.
...
class AllUserSubmissions(ConditionalUserTabMixin, UserMixin, SubmissionsListBase):
    ...
    def get_title(self):
        if self.is_own:
            return '내 모든 제출'
        return '%s의 모든 제출' % self.profile.display_name

    def get_content_title(self):
        if self.is_own:
            return '내 모든 제출'
        return mark_safe(escape('%s의 모든 제출') % (
...
class ProblemSubmissionsBase(SubmissionsListBase):
    ...
    def get_title(self):
        return '%s의 모든 제출' % self.problem_name
...
def single_submission(request):
    ...
    return render(request, 'submission/row.html', {
        ...
        'problem_name': submission.problem.translated_name(request.LANGUAGE_CODE),
...
class AllSubmissions(InfinitePaginationMixin, SubmissionsListBase):
    ...
    def use_infinite_pagination(self):
        return False
        # 무한 스크롤 방식은 사용하지 않고, 페이지네이션 방식만 사용하기 위해 주석 처리함
    ...
    def get_context_data(self, **kwargs):
        ...
        if contest:
            context['title_info'] = contest.name +  "의 제출 목록"
        context['dynamic_update'] = context['page_obj'].number == 1
        context['last_msg'] = event.last()
